...
def check_feature(df):  
    ...
    sns.regplot(x='erbmi', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='euffyday', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eufdsit', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eufastfdfrq', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eudrink', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='euexfreq', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='euexercise', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eustreason', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eumeat', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='eumilk', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x='euwic', y='body_composition', data=df)
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    pyplot.close('all')   
    sns.regplot(x="eusoda", y="body_composition", data=df);
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    pyplot.close('all')   
    sns.regplot(x="eusnap", y="body_composition", data=df);
    pyplot.show(fig)
    '''
    fig = pyplot.figure(figsize=(8,6))
    pyplot.close('all')   
    sns.regplot(x="erincome", y="body_composition", data=df);
    pyplot.show(fig)
    '''
    fig = pyplot.figure(figsize=(8,6))
    pyplot.close('all')   
    sns.regplot(x="eudietsoda", y="body_composition", data=df);
    pyplot.show(fig)
    
    fig = pyplot.figure(figsize=(8,6))
    pyplot.close('all')   
    sns.regplot(x="eufastfd", y="body_composition", data=df);
    pyplot.show(fig)
...
def convert_status_to_description(df):
    if df['body_composition'] == 0:
        return 0
    elif df['body_composition'] == 1:
        return 0
    elif df['body_composition'] == 2:
        return 1
    elif df['body_composition'] == 3:
        return 1      
...
def ensemble_voting_classifier(X_train, X_test, y_train, y_test, fold):
    ...
    #ensemble = BaggingClassifier(base_estimator = DecisionTreeClassifier(), n_estimators = 5, random_state = 7)
    ...
    #ensemble = AdaBoostClassifier(n_estimators = 50, random_state = 7)
    ensemble = GradientBoostingClassifier(n_estimators = 50, random_state = 7)
