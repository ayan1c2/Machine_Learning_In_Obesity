# -*- coding: utf-8 -*-
"""
Created on Sun Mar 15 22:10:55 2020
...
def check_feature(df):  
    ...
    #pyplot.close('all') 
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x="age", y="charges", data=df, color="g");
    pyplot.show(fig)
    ...
    #pyplot.close('all')   
    fig = pyplot.figure(figsize=(8,6))
    sns.regplot(x="smoker", y="charges", data=df, color="r");
    pyplot.show(fig)
    
    #pyplot.close('all')   
    sns.regplot(x="smoker", y="bmi", data=df);
    pyplot.show(fig)
...
def calculate_body_composition(df):
    if df['bmi'] < 18.5:
        return 0
    elif df['bmi'] >= 18.5 and df["bmi"] < 24.986:
        return 1
    elif df['bmi'] >= 25 and df['bmi'] < 29.926:
        return 2
    elif df['bmi'] >= 30:
        return 3   
'''  
...
def RF_regressor_accuracy(X_train, X_test, y_train, y_test, feature_cols, fold):
    ...
    results = cross_val_score(regressor, X_train, y_train, cv = fold)
...
def ensemble_voting_classifier(X_train, X_test, y_train, y_test, fold):
    ...
    ensemble = GradientBoostingClassifier(n_estimators = 150, random_state = 7)
    results = cross_val_score(ensemble, X_train, y_train, cv = fold)
    print ("aaaaaaaaaaaaaaaaaaa")
    print(results.mean()*100, results.std())
    print ("aaaaaaaaaaaaaaaaaaa")
    
def ensemble_voting_regressor(X_train, X_test, y_train, y_test, fold):
    from sklearn.ensemble import ExtraTreesClassifier
    ...
    models.append(('Bayes', linear_model.BayesianRidge()))
...
def plot_ml_model(X, y, fold):
    ...
    clfs = [SVC(kernel='linear'),
            ...
            #BernoulliNB(),
            ComplementNB(),
            DecisionTreeClassifier(criterion="entropy", max_depth = 24, min_samples_split = 2),
            DecisionTreeClassifier(criterion="entropy", max_depth = 24, min_samples_split = 2),
